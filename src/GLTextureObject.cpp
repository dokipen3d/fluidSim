...
GLTextureObject::~GLTextureObject() { glDeleteTextures(1, &textureID); }
...

{
  ...
  if (filePath.size() == 0) // for fbo drawing
  {
    if (textureType == GL_TEXTURE_RECTANGLE) {
      ...
      glTexImage2D(textureType, 0, GL_RGBA32F, 4, 4, 0, GL_RGB, GL_FLOAT,
                   NULL); // default size
    ...
    if (textureType == GL_TEXTURE_3D) {
      ...
      glTexImage3D(textureType, 0, GL_R32F, 64, 64, 64, 0, GL_RED, GL_FLOAT,
                   NULL);
      CheckGLError("tex image");
    }
  }
    ...
    // do custom loading here inclusing resetting size based on loaded file.
    // reset function below is only for fbo stuff
  }
}
...
GLuint GLTextureObject::getTextureID() { return textureID; }

GLenum GLTextureObject::getTextureType() { return textureType; }

void GLTextureObject::setRenderTargetMode(bool mode) { fboMode = mode; }

void GLTextureObject::setType(GLenum type) { textureType = type; }

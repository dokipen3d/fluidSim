...
protected:
    ...
    GridObject* parentGridObject = nullptr;//non owning pointer to parent for finding out chunksize.
    /* data */
    std::unique_ptr<LockFreeHashTable> chunks;

    float (ChannelObject::*SamplerPointerFunc[3])(float x, float y, float z, uint32_t channel);
    uint32_t currentChannelToSample;//for when we overload interpolate, we need to set this so we can iterate throught the chunks and return a vec 3

...
public:
    ...
    virtual void Algorithm(glm::i32vec3 chunkId, glm::i32vec3 voxelLocalPosition, Chunk* inChunk, Chunk* outChunk, u_int32_t dataIndex, uint32_t channel) = 0; //pure virtual. out chunk might not be needed but if we write a node that copies into a new grid we need to know where
    virtual void IterateGrid();
    virtual void PostChunkOp(Chunk*& inChunk, Chunk*& outChunk, glm::i32vec3 chunkIdSecondary);
...
protected:
	...
	ChannelObject* currentTargetChannelObject;//might make this a vector of pointers to dynamically add channels and pick from multiple

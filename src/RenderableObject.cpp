...
void RenderableObject::setupObjects()
{
    ...
    textureA = std::make_unique<GLTextureObject>("",GL_TEXTURE_RECTANGLE);//path to texture or blank if going to be rendering into it
...
//    cout << "bbox is " << bbox.min.z << " to " << bbox.max.z << endl;
    ...
    updateQuad(bbox.min.x, bbox.min.y, bbox.min.z, bbox.max.x, bbox.max.y, bbox.max.z);
    ...
    pos->uploadData(108 * sizeof (float), positions);//this binds the buffer for us under the hood. this binds the buffer and stores it in the vao
    vao->setVertexAttributeDataPointer(*pos, shader->getAttributeLocation("position"),3);//enables vertex array attributes and sets up pointer data

    col->uploadData(108 * sizeof (float), colours);
    vao->setVertexAttributeDataPointer(*col, shader->getAttributeLocation("colour"),3);
    vao->unBind();
    
    // projectionMatrixLocation2 = glGetUniformLocation(shader->getProgramID(), "projectionView2"); // Get the location of our projection matrix in the shader
    // cout << "projection location2 = " << projectionMatrixLocation2 << endl;
    projectionMatrixLocation = glGetUniformLocation(shader->getProgramID(), "projectionView"); // Get the location of our projection matrix in the shader
    //cout << "projection location = " << projectionMatrixLocation << endl;
    projectionMatrixLocation2 = glGetUniformLocation(volumeShader->getProgramID(), "projectionView"); // Get the location of our projection matrix in the shader
    //cout << "projection location2 = " << projectionMatrixLocation2 << endl;
    ...
    texLocVolume = glGetUniformLocation(volumeShader->getProgramID(), "volumeTexture");
    ...
    glBindVertexArray (vao);//bind array to start recording state of what buffers are used and whats in them
    glBindBuffer (GL_ARRAY_BUFFER, vbo);//bind this buffer. its bind state gets stored in the above vao
    cout << "buffer data 1" << endl;

    glBufferData (GL_ARRAY_BUFFER, 108 * sizeof (float), positions, GL_STATIC_DRAW);//upload data to buffer for position
    GLint location1 = shader->getAttributeLocation("position");//get location of input in shader
    glVertexAttribPointer (location1, 3, GL_FLOAT, GL_FALSE, 0, NULL);//set vertex pointer offset info for CURRENTLY BOUND BUFFER
...
//                {
                    //float sample =
                    threeD[i+(j*res)+(k*res*res)] = sampleObject->SampleTrilinear(posX, posY,posZ, 0);
                    //cout << sample << " ";
                //}

//                else
//                {
//                    float sample = sourceVolume->sampleVolume(glm::vec3(posX, posY,posZ));
...
void RenderableObject::Render(glm::mat4 ProjectionViewMatrix)
{
    ...
    glUniformMatrix4fv(projectionMatrixLocation, 1, GL_FALSE, glm::value_ptr(renderMat));
   ...
   // textureA->bind();
    ...
    volumeShader->bind();
    glUniformMatrix4fv(projectionMatrixLocation2, 1, GL_FALSE, glm::value_ptr(renderMat));
...
void RenderableObject::updateQuad(float minX, float minY, float minZ, float maxX, float maxY, float maxZ)
{

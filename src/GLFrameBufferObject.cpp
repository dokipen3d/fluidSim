...
{
    glBindFramebuffer(GL_DRAW_FRAMEBUFFER, 0);//set back to standard opengl buffer
    CheckGLError("fbo unbind");


}

void GLFrameBufferObject::setType(GLenum type)
{
...
void GLFrameBufferObject::bindTexture(GLTextureObject &textureToBindTo)
{
    if (glIsTexture(textureToBindTo.getTextureID()))
    {
       textureToBindTo.bind();
        bind();
        glFramebufferTexture2D(frameBufferType, GL_COLOR_ATTACHMENT0, textureToBindTo.getTextureType(), textureToBindTo.getTextureID(), 0);

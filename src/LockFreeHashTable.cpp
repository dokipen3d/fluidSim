...
uintptr_t LockFreeHashTable::GetItem(uint32_t key)
{
    ...
    while(true)
    {
        ...
        //uint32_t memoizedHash = m_entries[idx].memhash.load(std::memory_order_relaxed);
...
void LockFreeHashTable::setDummyItem(void* dummyLocation)//make sure its set to what you expect!
{
    dummy = dummyLocation;
...
void LockFreeHashTable::ClearKey(uint32_t key)
{
    ...
    while(true)
    {
        ...
        if (probedKey == key && m_entries[idx].value.compare_exchange_weak(oldVal,0))
        {
            m_entries[idx].value = 0;
            //m_entries[idx].key.store(0);//set key to 0. make sure item was deleted by owner otherwise data will leak
            break;
...
//        }
        ...
        if (probedKey == key )
        {
            if((m_entries[idx].value != 0) && m_entries[idx].value != (uintptr_t)dummy)
                return true;
            else
                return false;

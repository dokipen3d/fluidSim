...
public:
    ...
    RenderableObject(std::string newName, GridObject* inGridObject) : BaseObject(newName)
		{
            //std::cout << "in render object " << newName << " over loaded constructor" << std::endl;
	...
	void updateQuad(float minX, float minY, float minZ, float maxX, float maxY, float maxZ);

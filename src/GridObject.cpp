...
//channels are only created properly in reset grid. if we want to add another channel, then we need to add a channel info item and resetGrid will create it for us.
...
//            {
//                    channelObjs[(channelDetails["density"].memoryIndex)]->CreateChunk(i,0,0);
//                    bounding_box_.fluidMax = glm::i32vec3(	glm::max(bounding_box_.fluidMax.x, i),
//                                                        glm::max(bounding_box_.fluidMax.y, 1),
//                                                        glm::max(bounding_box_.fluidMax.z, 1)
//                                                    );
//                    bounding_box_.fluidMin = glm::i32vec3(	glm::min(bounding_box_.fluidMin.x, i),
//                                                        glm::min(bounding_box_.fluidMin.y, 1),
//                                                        glm::min(bounding_box_.fluidMin.z, 1)
//                                                    );
...
{
    channelDetails[inInfo.channelName] = ChannelInfo(numChannels, inInfo.channelName, inInfo.channelType);
    numChannels++;
...
//----------------------------------------------
void GridObject::SwapChannelPointers(std::string nameToSwap)//change direction of writing to
{
    ...
    std::swap(channelObjs.at(channelIndexToSwap), channelObjs.at(channelIndexToSwap+1));
    //cout << " swapping " << channelObjs.at(channelIndexToSwap).get() << " with " << channelObjs.at(channelIndexToSwap+1).get() << endl;
...
void GridObject::SetDefaultChannels()
{
    ...
    //scalarChannelInit = {string("density"),string("temperature"),string("fuel"),string("pressure") };
    ...
    {
        channelDetails[scalarChannelInit.at(i)] = ChannelInfo(channelIndex, scalarChannelInit.at(i),ChannelType::scalar);
    ...
    {
        channelDetails[vectorChannelInit.at(i)] = ChannelInfo(channelIndex, vectorChannelInit.at(i),ChannelType::vector);
        channelIndex++;
        channelIndex++;
    ...
    {
        channelDetails[sdfChannelInit.at(i)] = ChannelInfo(channelIndex, sdfChannelInit.at(i),ChannelType::sdf);
        channelIndex++;
        channelIndex++;

        numChannels++;	}

    myString << "channel defaults set up as no custom channels were specified. numchannel is " << numChannels << std::endl;
}
